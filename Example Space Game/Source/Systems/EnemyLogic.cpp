...
bool JK::EnemyLogic::Init(std::shared_ptr<flecs::world> _game,
	...
	game->system<Enemy, Health>("Enemy System")
		.each([this](flecs::entity e, Enemy, Health& h) {
			...
			if (e.get<Health>()->value <= 0) {
				// play explode sound
				e.destruct();
				JK::PLAY_EVENT_DATA x;
				GW::GEvent explode;
				explode.Write(JK::PLAY_EVENT::ENEMY_DESTROYED, x);
...
bool JK::EnemyLogic::Activate(bool runSystem)
{
	if (runSystem) {
